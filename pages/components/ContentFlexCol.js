...
function ContentFlexCol({ title, titleCss, children, src }) {
  return (
    <Container>
      <div className="flex lg:flex-row sm:flex-col items-start justify-between w-full gap-x-14 sm:gap-y-8 xs:flex-col xs:gap-y-8">
        {src && (
          <Image
            className="h-auto lg:w-[50%] sm:w-auto xs:w-auto rounded-xl "
            src={src}
